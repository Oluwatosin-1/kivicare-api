...
function isBoolean($value) {
 ...
 function kcaDoctorPatientList(){
    global  $wpdb;
    $appointment_table  = $wpdb->prefix. 'kc_' . 'appointments';
    $encounter_table    = $wpdb->prefix. 'kc_' . 'patient_encounters';
    $userid  = get_current_user_id();
    $query = " SELECT * FROM {$appointment_table} WHERE doctor_id = {$userid}";
    $appointments = collect($wpdb->get_results( $query))->pluck('patient_id')->unique()->toArray();
    $get_doctor_patient = collect($wpdb->get_results("SELECT *  FROM {$wpdb->base_prefix}usermeta WHERE `meta_value` = " . $userid . " AND
			                `meta_key` LIKE 'patient_added_by'"))->pluck('user_id')->unique()->toArray();

    $all_user = array_merge($get_doctor_patient, $appointments);
    $encounter = $wpdb->get_results( " SELECT * FROM {$encounter_table} WHERE doctor_id = {$userid}");
    $encounters = collect( $encounter)->pluck('patient_id')->unique()->toArray();
